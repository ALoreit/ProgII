package Algorithmen_Datenstrukturen.Aufgabe14_17;

public class ListeDL<T extends Comparable<T>> {
	...
	public void loescheDuplikate() {
		ElementDL<T> compare = head;
		while(compare != null){
			ElementDL<T> hilf = compare.next;
			while(hilf != null) {
				if(hilf.element.equals(compare.element)) {
					remove(hilf);
				}
				hilf = hilf.next;
			}
